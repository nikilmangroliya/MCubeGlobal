...
function initNikilApp() {
  ...
  // --------------------------------------------------------------------------
  // 0. STICKY NAVBAR — scroll shadow
  // --------------------------------------------------------------------------
  const siteHeader = document.getElementById('siteHeader');
  if (siteHeader) {
    window.addEventListener('scroll', () => {
      if (window.scrollY > 4) {
        siteHeader.style.boxShadow = '0 2px 16px 0 rgba(0,0,0,0.08)';
      } else {
    ...
    if (searchModal) {
      ...
      if (searchModalCard) {
        requestAnimationFrame(() => {
          searchModalCard.style.opacity = '1';
          searchModalCard.style.transform = 'translateY(0)';
        });
      }
  ...
  function closeSearch() {
    ...
    if (searchModalCard) {
      searchModalCard.style.opacity = '0';
      searchModalCard.style.transform = 'translateY(-14px)';
    }
    setTimeout(() => {
  ...
  // --------------------------------------------------------------------------
  // 3. CALCULATOR MANUAL ENTER ENGINE
  // --------------------------------------------------------------------------
  const calcBudgetInput = document.getElementById('calcBudgetInput');
  const calcCacInput = document.getElementById('calcCacInput');
  ...
  function updateHeroSimulator() {
    if (!calcBudgetInput || !calcCacInput) return;

    const monthlySpend = Math.max(0, parseFloat(calcBudgetInput.value) || 0);
    const currentCac = Math.max(0, parseFloat(calcCacInput.value) || 0);
    ...
    const baseIroas = 3.8;
    const spendBonus = (monthlySpend / 50000000) * 0.7;
    const iRoas = (baseIroas + spendBonus).toFixed(2);

    if (heroBudgetValue) heroBudgetValue.textContent = 'Formatted: ' + formatCurrency(monthlySpend) + ' / mo';
    if (heroCacValue) heroCacValue.textContent = 'Formatted: ' + formatCurrency(currentCac) + ' / customer';
  ...
  if (calcBudgetInput && calcCacInput) {
    calcBudgetInput.addEventListener('input', updateHeroSimulator);
    calcCacInput.addEventListener('input', updateHeroSimulator);
    updateHeroSimulator();
  }
